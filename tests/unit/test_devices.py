...
class DevicesTestCase(unittest.TestCase):
    ...
    def test__hwinfo_parse_short(self):
        hwinfo = textwrap.dedent('''
            ...
            unknown:
              ...
              /dev/ttyS0           16550A
        ''')
        self.assertEqual(devices._hwinfo_parse_short(hwinfo), {
            'cpu': {
                0: 'QEMU Virtual CPU version 2.5+, 3591 MHz',
            },
            'keyboard': {
                '/dev/input/event0': 'AT Translated Set 2 keyboard',
            },
    ...
    def test__hwinfo_parse_full_floppy(self):
        ...
        ''')
        self.assertEqual(devices._hwinfo_parse_full(hwinfo), {
            '01': {
            ...
            '02': {
                ...
                'Drive status': 'no medium',
                'Attached to': {
                    'Handle': '#1 (Floppy disk controller)',
                },
            },
        })
    ...
    def test__hwinfo_parse_full_unknown(self):
        ...
        self.assertEqual(devices._hwinfo_parse_full(hwinfo), {
            ...
            '06': {
                ...
                'Hardware Class': 'unknown',
                'Model': 'DMA controller',
                'I/O Ports': ['0x00-0xcf7 (rw)', '0xc0-0xdf (rw)', '0x80-0x8f (rw)'],
                'DMA': '4',
                'Config Status': {
                    'cfg': 'new',
                    'avail': 'yes',
    ...
    def test__hwinfo_parse_full_storage(self):
        ...
        self.assertEqual(devices._hwinfo_parse_full(hwinfo), {
            '17': {
                ...
                'Driver': ['ata_piix'],
                'Driver Modules': ['ata_piix'],
                'I/O Ports': ['0x1f0-0x1f7 (rw)', '0x3f6 (rw)', '0x170-0x177 (rw)',
                              '0x376 (rw)', '0xc020-0xc02f (rw)'],
                'Module Alias': 'pci:v00008086d00007010sv00001AF4sd00001100bc01sc01i80',
                'Driver Info #0': {
                    'Driver Status': 'ata_piix is active',
                    'Driver Activation Cmd': 'modprobe ata_piix',
    ...
    def test__hwinfo_parse_full_disk(self):
        ...
        self.assertEqual(devices._hwinfo_parse_full(hwinfo), {
            '21': {
                ...
                'Driver Modules': ['ata_piix', 'sr_mod'],
                'Device File': '/dev/sr0 (/dev/sg1)',
                'Device Files': ['/dev/sr0', '/dev/cdrom', '/dev/dvd',
    ...
    def test__hwinfo_parse_full_cpu(self):
        ...
        self.assertEqual(devices._hwinfo_parse_full(hwinfo), {
            '27': {
                ...
                'Vendor': 'GenuineIntel',
                'Model': '6.6.3 "QEMU Virtual CPU version 2.5+"',
                'Features': ['fpu', 'de', 'pse', 'tsc', 'msr', 'pae',
                             'mce', 'cx8', 'apic', 'sep', 'mtrr',
                             'pge', 'mca', 'cmov', 'pse36', 'clflush',

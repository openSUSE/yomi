...
class SUSEConnectTestCase(unittest.TestCase):
    ...
    def test_register_error(self):
        ...
        '''
        result = {'retcode': 1, 'stdout': 'Unknown Registration Code',
                  'stderr': ''}
        salt_mock = {
            'cmd.run_all': MagicMock(return_value=result),
        }
    ...
    def test_status_params(self):
        ...
        with patch.dict(suseconnect.__salt__, salt_mock):
            self.assertEqual(suseconnect.status(root='/mnt'),
                             [{
                                 'identifier': 'SLES',
                                 'version': '15.2',
                                 'arch': 'x86_64',
    ...
    def test__parse_list_extensions(self):
        ...
        fixture = os.path.join(os.path.dirname(__file__),
                               'fixtures/list_extensions.txt')
        with open(fixture) as f:
            self.assertEqual(suseconnect._parse_list_extensions(f.read()), [
    ...
    def test_cleanup(self):
        ...
        with patch.dict(suseconnect.__salt__, salt_mock):
            self.assertEqual(suseconnect.cleanup(),
    ...
    def test_cleanup_params(self):
        ...
        with patch.dict(suseconnect.__salt__, salt_mock):
            self.assertEqual(suseconnect.cleanup(
                root='/mnt'), 'Service has been removed')
            salt_mock['cmd.run_all'].assert_called_with(
                ['SUSEConnect', '--cleanup',
                 '--root', '/mnt'])
    ...
    def test_rollback_params(self):
        ...
        with patch.dict(suseconnect.__salt__, salt_mock):
            self.assertEqual(suseconnect.rollback(
                url='https://scc.suse.com',
                root='/mnt'), 'Starting to sync system product activations')
            salt_mock['cmd.run_all'].assert_called_with(
                ['SUSEConnect', '--rollback',
                 '--url', 'https://scc.suse.com',
                 '--root', '/mnt'])
